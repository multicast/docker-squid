...
![Pulls](https://img.shields.io/docker/pulls/mkovac/squid.svg)
![Stars](https://img.shields.io/docker/stars/mkovac/squid.svg)
...
Run prebuilt:
    ...
    $ docker run --rm -ti mkovac/squid bash

Or you can clone & build, run `bash` to explore:

    $ git clone https://github.com/multicast/docker-squid
    $ cd docker-squid
    $ docker build -t squid .
    $ docker run --rm -ti squid bash

## Compose snippet

    squid:
      image: mkovac/squid:latest
      environment:
        #SQUID_PORT: 3128
        #BASE_DEBUG: 9
...
# Packages
    ...
    ii  squid                         4.6-1+deb10u10              amd64        Full featured Web Proxy cache (HTTP proxy)
    ii  squid-common                  4.6-1+deb10u10              all          Full featured Web Proxy cache (HTTP proxy) - common files
